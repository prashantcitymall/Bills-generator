...
const pgConfig = {
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/billgenerator',
...
const bills = {
  ...
  async findByUserId(userId) {
    try {
      ...
      return await db.any('SELECT * FROM bills WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
    } catch (error) {
  ...
  async findById(id, userId) {
    try {
      ...
      return await db.oneOrNone('SELECT * FROM bills WHERE id = $1 AND user_id = $2', [id, userId]);
    } catch (error) {
  ...
  async update(id, userId, updates) {
    try {
      ...
      if (!bill) {
        logger.error(`Bill ${id} not found or doesn't belong to user ${userId}`);
        return null;
      }
      
      const allowedFields = [
        'customer_name', 'customer_address', 'customer_phone',
        'total_amount', 'payment_status', 'items', 'notes'
      ];
  ...
  async delete(id, userId) {
    try {
      ...
      const result = await db.result('DELETE FROM bills WHERE id = $1 AND user_id = $2', [id, userId]);
